...
void oneColorTransformations(
    ...
    #pragma omp for schedule(dynamic)
        for (size_t iter = 0; iter < total_iterations; ++iter) {
            ...
            path = prefix0 + std::to_string(tole) + p.weightedColors;
            path += suffix;
...
void complete_transformations_by_proportion(
    ...
    constexpr size_t num_transforms = transformation_params.size();
    const int num_threads = computeNumThreads();
    #pragma omp parallel for collapse(3) schedule(dynamic) num_threads(num_threads) \
    default(none) \
    shared(prop_values, transformation_params, total_step_by_step_output_dirs, total_step_by_step_suffixes, colorNuances, \
    baseImage, baseName) \
    firstprivate(num_props, num_transforms)
    for (size_t p_idx = 0; p_idx < num_props; ++p_idx) {
        for (size_t transformIdx = 0; transformIdx < num_transforms; ++transformIdx) {
            ...
            for (int colorNuance = colorNuances[0];
                ...
                modified.get().proportion_complete(p, colorNuance, dark, below);
...
void partial_transformations_by_proportion(
    ...
    std::vector<int> rectangles = range_to_vector(first_and_last_rectangles);
    ...
    #pragma omp parallel for collapse(3) schedule(dynamic) num_threads(num_threads) \
    default(none) \
    shared(prop_values, transformation_params, colorNuances, \
    ...
    for (size_t p_idx = 0; p_idx < num_props; ++p_idx) {
        for (size_t transformIdx = 0; transformIdx < num_transforms; ++transformIdx) {
            ...
            for (int colorNuance = colorNuances[0];
                ...
                modified.get().proportion_region_fraction(p, colorNuance, fraction, rectangles, dark, below);
...
void processImageTransforms(
    ...
    if (partial) {
        partial_transformations_by_proportion(
            image, baseName, proportions, rectangles, colorNuances, fraction);
    }

    if (partialInDiagonal) {
        const std::vector<int> rectanglesInDiagonal = genererRectanglesInDiagonal(fraction);
        partial_transformations_by_proportion(
