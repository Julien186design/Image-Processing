...
struct parameters {
    static constexpr std::array<float, 3> proportions = {0.F, 1.F, .03125F};
    ...
    static constexpr std::array<int, 3> toleranceOneColor = {0, 20, 20};
    static constexpr std::array<float, 3> weightOfRGB = {.1F, 1.F, .15F};
    static constexpr std::array<float, 3> passesRGB = {1.F, 1.F, .25F}; // 0 = original image, 1 = colored image
    ...
    static constexpr int numTolerance = (toleranceOneColor[1] - toleranceOneColor[0]) / toleranceOneColor[2] + 1;
    static constexpr int iterationsRGB = static_cast<int>((weightOfRGB[1] - weightOfRGB[0]) / weightOfRGB[2]);
...
public:
    ...
    static auto image_reverse(const std::string& outputDir,
                               const std::string& baseName,
                               const std::string& suffix,
                               const float proportion) -> std::string {
    ...
    static std::string writingWeightedColors(const std::span<const float> weightOfRGB) {
        if (weightOfRGB[0] == weightOfRGB[1]) {
...
#endif //IMAGE_PROCESSING_PROCESSINGCONFIG_H#ifndef IMAGE_PROCESSING_PROCESSINGCONFIG_H
#define IMAGE_PROCESSING_PROCESSINGCONFIG_H
