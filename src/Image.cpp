...
[[nodiscard]] std::vector<Image> Image::simplify_to_dominant_color_combinations(
    ...
    const size_t passes = tValues.empty()
        ? parameters::numProportionSteps
        : tValues.size();

    std::vector results(passes, *this);

	const size_t pixelCount = (size >= channels) ? (size - (channels - 1)) : 0;

    // Parallelise over pixels: each pixel across all passes is independent.
    // No false sharing risk since each thread works on a different pixel index.
    #pragma omp parallel for schedule(static) \
        default(none) shared(results, weightOfRGB, tValues) \
        firstprivate(passes, tolerance, pixelCount)
    for (size_t i = 0; i < pixelCount; i += channels) {
        for (size_t passe = 0; passe < passes; ++passe) {
            uint8_t& red = results[passe].data[i];
            uint8_t& green = results[passe].data[i + 1];
            uint8_t& blue = results[passe].data[i + 2];
